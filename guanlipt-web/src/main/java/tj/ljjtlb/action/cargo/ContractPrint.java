...
import tj.ljjtlb.service.ContractserviceImpl;
import tj.ljjtlb.utils.DownloadUtil;
import tj.ljjtlb.utils.UtilFuns;
import tj.ljjtlb.utils.file.PoiUtil;
...
public class ContractPrint{
	...
	public void print(Contract contract,String path, HttpServletResponse response) throws Exception{
		//相同厂家的信息一起打印
		List<ContractProduct> cptlists=new ArrayList<>();
		Set<ContractProduct> oList = contract.getContractProducts();
		for (ContractProduct contractProduct : oList) {
			cptlists.add(contractProduct);
		}
